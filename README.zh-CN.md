...
当你希望 KRA 引导完成 provider 创建、认证配置、registry 更新、URL 打印以及启动本地 server 时，请使用交互模式。当尚未配置 provider 时，向导会从最小化设置流程开始。至少存在一个 provider 后，主菜单可以管理完整生命周期。
...
| 包                                                     | 用途                                                                |
| ------------------------------------------------------ | ------------------------------------------------------------------- |
| [`@kastral/kra`](./packages/cli/README.zh-CN.md)       | 已发布的 CLI，用于交互模式、命令模式、一次性 `npx` 执行和 `serve`。 |
...
安装 git 时，KRA 会提交成功的 `add`、`update` 和 `remove` 状态变更。如果未安装 git，KRA 仍会使用 `.internal/state.json` 正常更新和合并 registry。如果你手动编辑 `api.json`，请先校验 JSON 结构后再用 git 提交；KRA 在加载 registry 并提供服务时也会校验 registry 文件。后续 `kra update` 会使用 `.internal/state.json.lastGeneratedRegistry` 作为合并基线，因此手动提交不会变成生成基线。
